...
import { businessLocalUrl } from '@/config/regionalNavigation';
...
export default function BusinessPage({ pageKey, prerender = false }: { pageKey: BusinessPageKey; prerender?: boolean }) {
  ...
  return <>
    ...
    <main lang="es" className="pt-20 text-gray-900">
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8"><h2 className="text-3xl font-bold">Cómo funciona</h2><ol className="mt-9 grid gap-8 md:grid-cols-3">{[['Cuéntanos qué necesitas', 'Indica el municipio, el tipo de instalación, la superficie aproximada y la frecuencia que necesitas.'], ['Valoramos el alcance', 'Revisamos zonas, accesos y horarios para preparar la propuesta. Si hace falta conocer la instalación, acordamos una visita de valoración.'], ['Organizamos el servicio', 'Una vez acordadas las condiciones, planificamos el inicio, el equipo y el seguimiento de las tareas contratadas.']].map(([title, text], i) => <li key={title}><span className="text-sm font-bold text-emerald-700">0{i + 1}</span><h3 className="mt-3 text-xl font-bold">{title}</h3><p className="mt-3 leading-relaxed text-gray-600">{text}</p></li>)}</ol>
        <p className="mt-8 max-w-3xl leading-relaxed text-gray-600">¿Necesitas revisar el alcance antes de pedir presupuesto? <Link to="/contacto" className="font-semibold text-emerald-800 underline underline-offset-4 hover:text-emerald-950">Contacta con Superclim</Link> para explicar las necesidades de tu instalación y valorar si conviene una visita.</p>
        <div className="mt-16 grid gap-8 md:grid-cols-2"><article className="rounded-3xl bg-emerald-50 p-8"><ClipboardList className="h-9 w-9 text-emerald-700" aria-hidden="true" /><h2 className="mt-5 text-2xl font-bold">Qué queda definido en la propuesta</h2><p className="mt-4 leading-relaxed text-gray-700">Zonas, tareas, frecuencia, horarios y condiciones de acceso. También quedan definidos los materiales previstos en la propuesta y las responsabilidades de cada parte, para que el servicio pueda revisarse sobre un alcance claro.</p></article><article className="rounded-3xl border border-gray-200 p-8"><Users className="h-9 w-9 text-emerald-700" aria-hidden="true" /><h2 className="mt-5 text-2xl font-bold">Organización y control del servicio</h2><p className="mt-4 leading-relaxed text-gray-700">Superclim organiza su equipo, los turnos, la supervisión y los materiales previstos en la propuesta para ejecutar el servicio contratado. Gestionamos las incidencias y las sustituciones cuando corresponda, según las condiciones acordadas, y realizamos el seguimiento del cumplimiento de las tareas y frecuencias del contrato.</p></article></div>
      </section>
      <section className="bg-emerald-950 py-16 text-white"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><h2 className="text-3xl font-bold">Desde Sabadell, para el Vallès Occidental y Barcelona</h2><p className="mt-5 max-w-3xl leading-relaxed text-emerald-100">Indícanos la ubicación de tu centro de trabajo para valorar desplazamientos, accesos y horarios en la propuesta.</p><ul className="mt-7 flex flex-wrap gap-3">{businessCoverage.map(city => {
        const href = pageKey === 'officeCleaning' ? businessLocalUrl('oficinas', city) : pageKey === 'industrialCleaning' ? businessLocalUrl('naves', city) : undefined;
        return <li key={city} className="rounded-full border border-emerald-700 px-4 py-2">{href ? <Link to={href} className="rounded underline underline-offset-4 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white">{city}</Link> : city}</li>;
      })}</ul></div></section>
